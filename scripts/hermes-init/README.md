...
`hermes-init.sh` 会写入 `/etc/profile.d/hermes-shell.sh`，并在 `/root/.bashrc` 追加一个受控加载块。配置只对 root 的交互 bash 生效：
...
| `/root/.bashrc` | 加载 `/etc/profile.d/hermes-shell.sh`，让普通交互 bash 立即获得增强 |
...
- root dotfiles、profile、completion、`hermes-setup` 会被重新写入；`/root/.bashrc` 只追加一次 hermes-init 加载块。
...
- `hermes-init.sh` 不覆盖 `/root/.bashrc`，只在缺少加载块时追加。
