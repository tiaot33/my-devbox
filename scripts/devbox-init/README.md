...
```bash
sudo bash <(wget -qO- https://raw.githubusercontent.com/tiaot33/my-devbox/main/scripts/devbox-init/install.sh)
```

终端里会在 bootstrap 之前问要装哪些 coding agents，直接回车 = 一个都不装。已经想好、或不想被问：

```bash
DEVBOX_AGENTS=claude,omp,grok bash <(wget -qO- https://raw.githubusercontent.com/tiaot33/my-devbox/main/scripts/devbox-init/install.sh)
bash scripts/devbox-init/install.sh --agents claude,codex
bash scripts/devbox-init/install.sh --no-agents
```
...
```bash
bash scripts/devbox-init/install.sh
# 或
sudo bash scripts/devbox-init/install.sh
```
