...
- **路径库（path library）** — 许多文档挂在**同一个**项目下，用 URL 路径区分（`/d/<id>/`）。第一份就要按这个形状落地：事后把根路径的 `index.html` 挪进 `d/<id>/`，已经分享出去的 URL 会断。
...
5. **登录交给本人** — 确认之后先做该平台的状态探测。未登录就把 login 命令交给用户跑，等他们回来再继续。`wrangler login` / `vercel login` / `netlify login` / `surge login` / `gh auth login` 都是交互式的，替用户跑会卡住。
...
存续是长期、或来源是 AI 文档时，按路径库起铺——哪怕手里只有一份。根路径只留给用户说清「就这一页、不会再加」。

**完成标准：** 上表每个信号已知或已明确采用默认值；长期 / AI 文档已按路径库起铺（第一份也落在 `d/<id>/`）。
...
| 约束 | 影响 |
| --- | --- |
| 长期归档或 AI HTML（哪怕现在一份） | CF Pages（或 Netlify）**路径库**，第一份就放进 `d/<id>/`。CF 上「一文件一项目」会撞上账号 100 个项目上限。 |
...
**完成标准：** 用户无需再调研就能发布；长期 / AI 文档的配方把第一份放在 `d/<id>/`（用户明确只要单页根路径时除外）；任何已执行的发布动作都先经用户明示确认。

## 默认策略

用户只说「把这个 HTML 放到网上」、未说明形态时：

1. 单文件或小目录、**明确临时**（几小时/几天）→ **Vercel Drop** 或 **CF Pages 上传**。
2. AI HTML，或要长期打开 → **Cloudflare Pages 路径库**（第一份就起铺，不必等「每天 10 份」）。
3. 已是公开 GitHub 仓库 → **GitHub Pages**。

## 路径库布局（内联 — 长期 / AI 文档从第一份起铺）
...
接到已有库：先看本地是否已有 `site/d/`，以及上次的固定项目名（用户说过的名字、`.surge-domain`、`wrangler pages project list`）。接到同一棵树，不要另起项目。
